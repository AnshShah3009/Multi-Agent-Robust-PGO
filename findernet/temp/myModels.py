...
class end2endModel(nn.Module):
    ...
    def DYT(self, emb1, emb2):
        ...
        vs, us = torch.meshgrid([vrange, urange], indexing=None)
        xs = us * torch.cos(vs) 
        ys = us * torch.sin(vs)
        ...
        for batch in range(batchSize):
            ...
            thetas[batch, 0] = torch.sum(torch.mul(angleWeights[:,0,:,0], binAngles))
    ...
    def pixelwiseDist(self, emb1, emb2):
        batchSize, s = emb1.size()

        arranged = emb1.unsqueeze(1).unsqueeze(1)
        arranged2 = emb2.unsqueeze(1).unsqueeze(1)

        rows = torch.tile(arranged, (1,1,s,1))
        cols = torch.tile(arranged2.swapaxes(-1,-2), (1,1,1,s))
    ...
    def forward(self, dem1, dem2):
        ...
        # np.save("sampleKittiData/dem1.npy", dem1.cpu().detach().numpy())
        # np.save("sampleKittiData/dem2.npy", dem2.cpu().detach().numpy())
        # np.save("sampleKittiData/dem1_emb.npy", emb1.cpu().detach().numpy())
        # np.save("sampleKittiData/dem2_emb.npy", emb2.cpu().detach().numpy())
        ...
        # print("rec Dem shape: ", reconstructed_dem1.shape)
        ...
        # print("transformed emb: ", transformedEmb1.shape)
        ...
        # print("enriched shape: ", emb1.shape)

        pixelWiseMatrix = self.pixelwiseDist(emb1, emb2)
        
        # print("pixelwise shape: ", pixelWiseMatrix.shape)
