...
## -- generate DEM --
# d is the bucket size
def generateDEMFromPCD(canonicalPCD, G_w, G_h, d, demRange=80):
    DEM = np.ones([G_w, G_h])
    DEM *= -np.inf
    ...
    # print(f"(%f/%f), (%f/%f)" % (points[:,0].min(), points[:,0].max(), points[:,1].min(), points[:,1].max()))
    # print(f"X: (%f/%f) Y: (%f/%f), dists: (%f/%f)" % (minX, maxX, minY, maxY, Xdist, Ydist))
    ...
    for point in points:
        ...
        if (demLocation[0] >= G_h or demLocation[0] < 0) or (demLocation[1] >= G_w or demLocation[1] < 0):
            continue
...
# kitti demRange is 80
def generateDEM(binPath, demRange=80, randRot=False, vis=False):
    # read pcds
    pcd = readPCD(binPath)
    ...
    # IF HUSKY SEQUENCE:
    d = 0.025
    DEM, subPCD = generateDEMFromPCD(canonicalPCD, 500, 500, d, demRange=demRange)
